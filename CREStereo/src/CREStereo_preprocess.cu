#include "CREStereo_preprocess.h"
#include <opencv2/opencv.hpp>

__global__ void CREStereo_preprocess_kernel(uint8_t* src, int src_line_size, int src_width, int src_height, float* dst, int edge)
{
...
__global__ void CREStereo_reprojectImageTo3D_kernel(uint8_t* left_img,float*disparity,float*pointcloud,float*Q_device,int disparity_rows,int disparity_cols)
{
...
void CREStereo_preprocess(uint8_t* src, float* dst, int src_width, int src_height, cudaStream_t stream) 
{
    ...
    CREStereo_preprocess_kernel<<<blocks, threads, 0, stream>>>(src, src_width*3, src_width, src_height, dst, jobs);
}
...
void CREStereo_reprojectImageTo3D(uint8_t* left_img,float*disparity,float*pointcloud,float*Q_device,int disparity_rows,int disparity_cols)
{
    ...
    int threads=256;
    int blocks=(jobs+threads-1)/threads;
    CREStereo_reprojectImageTo3D_kernel<<<blocks,threads>>>(left_img,disparity,pointcloud,Q_device,disparity_rows,disparity_cols);
}